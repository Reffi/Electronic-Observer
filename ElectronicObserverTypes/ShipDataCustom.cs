...
namespace ElectronicObserverTypes
{
        ...
        IHitRateDefender, IAntiInstallationAttacker<EquipmentDataCustom>, IInstallation
    {
        ...
        private readonly EquipmentDataCustom[] _equipment = new EquipmentDataCustom[6];

        public VisibleFits CurrentFitBonus => Equipment.Where(eq => eq != null)
        ...
        public EquipmentDataCustom[] Equipment
        {
            ...
            set
            {
                value ??= new EquipmentDataCustom[6];

                if (value.Length < 6)
                {
                    List<EquipmentDataCustom> temp = value.ToList();
                    for (int i = temp.Count; i < 6; i++)
                    {
                        temp.Add(_equipment[i]);
                    }
                ...
                for (int i = 0; i < 6; i++)
                {
                    if (_equipment[i] == value[i]) continue;

                    _equipment[i] = value[i];

                    // if(value[i] == null) continue;

                    Equipment[i].CurrentFitBonus = new FitBonusCustom(this, value[i]);
                }
        ...
        public ShipDataCustom(IShipDataMaster ship)
        {
            SortID = ship.SortID;
            ...
            Level = IsSubmarine && IsAbyssalShip ? 50 : 1;


            List<EquipmentDataCustom> equip = new List<EquipmentDataCustom>();
            ...
            InstallationType = GetInstallationType(this);
        }
        ...
        private InstallationType GetInstallationType(ShipDataCustom ship) => (int)ship.ShipID switch
        {
            // 飛行場姫
        ...
        {
            // todo fix this
            IShipDataMaster ship = null; // = KCDatabase.Instance.MasterShips[ID];
        ...
        public bool IsExpansionSlotAvailable { get; }

        public string NameWithLevel => $"{Name} Lv. {Level}";

        public ShipClasses ShipClass { get; set; }
        ...
        // todo localize enum
        public string ShipTypeName => ShipType.Display(); // MasterShip.ShipTypeName;

        public bool IsMarried => Level > 99;

        public bool IsAbyssalShip => (int)ShipID > 1500;

        public bool IsInstallation => BaseSpeed == 0;

        public bool IsCarrier => ShipType switch
        {
            ShipTypes.LightAircraftCarrier => true,
            ShipTypes.AircraftCarrier => true,
            ShipTypes.ArmoredAircraftCarrier => true,

            _ => false
        };

        public bool IsSubmarine => ShipType switch
        {
